...
function formatBillNo(type, seq) {
  ...
  const prefixes = {
    purchase_order:
      process.env.COMPANY_PREFIX_PO || process.env.COMPANY_PREFIX || "INGPO",
    proforma_invoice:
      process.env.COMPANY_PREFIX_PI || process.env.COMPANY_PREFIX || "INGPF",
    invoice:
      process.env.COMPANY_PREFIX_INV || process.env.COMPANY_PREFIX || "INGINV",
  };

  const prefix = prefixes[type] || process.env.COMPANY_PREFIX || "INGPO";
...
async function getNextSequence(type = "purchase_order") {
  const id = `bill_${type}`; // separate counters per document type
...
const createBill = asyncHandler(async (req, res) => {
  ...
  // Determine bill type and generate type-aware sequence & number
  const billType =
    payload.type &&
    ["purchase_order", "proforma_invoice", "invoice"].includes(payload.type)
      ? payload.type
      : "purchase_order";

  const seq = await getNextSequence(billType);
  const poNo = formatBillNo(billType, seq);
  console.log("Generated Bill Number:", poNo, "for type", billType);
...
const listBills = asyncHandler(async (req, res) => {
  ...
  const [bills, total] = await Promise.all([
    Bill.find(filter)
      ...
      .limit(limit)
      .select(
        "poNo customerName grandTotal date pdfUrl createdAt imageUrl currency"
      ),
